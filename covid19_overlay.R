...
qp1 <- qp1 + geom_point(data = sodata, aes(x=Xo, y=Yo, color=country), color="red", size=psize)
...
qp1 <- qp1 + scale_y_continuous(breaks = pretty_breaks(n = 6), labels=comma, sec.axis = sec_axis(~., name=paste("Cases in",country), breaks = pretty_breaks(n = 6), labels=comma))
...
qp1 <- qp1 + annotate("text", label = paste(c("US cases: ",predcasesp," -"), collapse = ""), x = tdays-28, y = predcasesp, size=fsize*0.25)
