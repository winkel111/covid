...
modelfitp <- nls2(formula = Yp ~ I(a*exp(b*(Xp-c))),  data = spdata, start = list(a = 0.003, b = 0.33, c = 1), control = nls.control(maxiter = 1000, tol = 1e-02, minFactor = 1/1024, printEval = TRUE, warnOnly = TRUE), algorithm = "port")
modelfito <- nls2(formula = Yo ~ I(a*exp(b*(Xo-c))),  data = sodata, start = list(a = 0.003, b = 0.33, c = 1), control = nls.control(maxiter = 1000, tol = 1e-02, minFactor = 1/1024, printEval = TRUE, warnOnly = TRUE), algorithm = "port")
...
qp1 <- qp1 + scale_y_continuous(sec.axis = sec_axis(~., name=paste("Number of cases in",country)))
